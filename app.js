...
function App() {
    ...
    const fetchMonthlyWeeksData = async () => {
        try {
            ...
            const monthlyData = {
                callback: Array(weekCount).fill(0),
                approval: Array(weekCount).fill(0),
                invited: Array(weekCount).fill(0)
            };
            ...
            for (let i = 0; i < weekCount; i++) {
                ...
                window.currentStartDate = weekStart;
                window.currentEndDate = weekEnd;
    ...
    const getWeeklyLabels = () => {
        ...
        const weekCount = Math.ceil(diffDays / 7);
        return Array.from({length: weekCount}, (_, i) => `${i + 1}-я неделя`);
    };
    ...
    const prepareWeeklyChartData = (weeklyLeadsData) => {
        ...
        if (daysOrder.length > 14) {
            ...
            const weekCount = Math.ceil(daysOrder.length / 7);
            for (let week = 0; week < weekCount; week++) {
                const weekStart = week * 7;
                const weekEnd = Math.min(weekStart + 7, daysOrder.length);
                
                groupedResult.callback[week] = result.callback.slice(weekStart, weekEnd).reduce((sum, val) => sum + val, 0);
                groupedResult.approval[week] = result.approval.slice(weekStart, weekEnd).reduce((sum, val) => sum + val, 0);
                groupedResult.invited[week] = result.invited.slice(weekStart, weekEnd).reduce((sum, val) => sum + val, 0);
            }
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="dashboard-card mb-8">
                    <h2 className="text-xl font-semibold mb-6 text-gray-900">
                        Графики по дням ({window.currentStartDate ? window.currentStartDate.toLocaleDateString('ru-RU') : ''} - {window.currentEndDate ? window.currentEndDate.toLocaleDateString('ru-RU') : ''})
                    </h2>
                ...
                <div className="dashboard-card mb-8">
                    <h2 className="text-xl font-semibold mb-6 text-gray-900">
                        Сравнение по неделям ({window.currentStartDate ? window.currentStartDate.toLocaleDateString('ru-RU') : ''} - {window.currentEndDate ? window.currentEndDate.toLocaleDateString('ru-RU') : ''})
                    </h2>
                    
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="dashboard-card">
                            <WeeklyComparisonChart 
                                data={monthlyWeeksLeads.callback || []}
                        ...
                        <div className="dashboard-card">
                            <WeeklyComparisonChart 
                                data={monthlyWeeksLeads.approval || []}
                                labels={getWeeklyLabels()}
                                color="#f59e0b"
                                title="На согласовании"
                            />
                        ...
                        <div className="dashboard-card">
                            <WeeklyComparisonChart 
                                data={monthlyWeeksLeads.invited || []}
                                labels={getWeeklyLabels()}
                                color="#10b981"
                                title="Приглашен к рекрутеру"
                            />
